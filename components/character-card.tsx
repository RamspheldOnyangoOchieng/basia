...
export function CharacterCard({ character }: CharacterCardProps) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-[var(--radius)] aspect-[3/4] cursor-pointer shadow-md hover:shadow-lg transition-all duration-300 hover:shadow-[0_0_15px_rgba(165,94,191,0.3)]"
    ...
    >
      ...
      {/* NEW Badge */}
      {isNew && (
        <div className="absolute top-3 right-3 bg-[#a55ebf] text-white text-xs font-bold px-2 py-1 rounded-full flex items-center shadow-sm z-20">
          <svg
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 text-white z-20">
        <div className="flex items-baseline gap-1.5">
          ...
          {age && <span className="text-sm text-gray-300">{age}</span>}
        </div>
        <p className="text-xs text-gray-200 line-clamp-2 mt-0.5">{character.description}</p>
      </div>
    </div>
  )
}
