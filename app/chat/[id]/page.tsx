    ...
    try {
      ...
      try {
        ...
        response = await fetch("/api/img2img", {
          ...
          body: JSON.stringify({
            prompt: prompt,
            negativePrompt: "nsfw, bad quality, worst quality, low quality",
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-background">
      ...
      <div className="hidden md:block md:w-72 border-b md:border-b-0 md:border-r border-border flex flex-col md:h-full h-auto max-h-[40vh] md:max-h-none">
        ...
        <div className="p-4 border-b border-border">
          <div className="relative">
            ...
            <Input placeholder="Search for a profile..." className="pl-9 bg-card border-none" />
          </div>
        </div>
                ...
                <Link href={`/chat/${char.id}`} key={char.id} className="block">
                  <div
                    className={`flex items-center p-3 rounded-xl cursor-pointer ${
                      characterId === char.id ? "bg-[#252525] text-white" : "hover:bg-[#252525] hover:text-white"
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <h3 className="font-medium text-foreground truncate">{char.name}</h3>
      ...
      <div className="flex-1 flex flex-col h-screen md:h-full">
        {/* Chat Header */}
        <div className="h-14 md:h-16 border-b border-border flex items-center px-3 md:px-4 justify-between">
          <div className="flex items-center">
            <Button
              variant="ghost"
              size="icon"
        ...
        {/* Chat Messages */}
        <div className="flex-1 overflow-auto p-2 md:p-4 space-y-3 md:space-y-4">
            ...
            <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              >
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (currentlyPlayingMessageId === message.id) {
                          ...
                          generateSpeech(message.id, message.content)
                        }
                      }}
                      disabled={isGeneratingSpeech[message.id]}
                      title={currentlyPlayingMessageId === message.id ? "Stop" : "Play"}
                    >
                ...
                {message.isImage && message.imageUrl && (
                  <div className="mt-2 relative w-full aspect-square max-w-xs rounded-2xl overflow-hidden">
        ...
        <div className="p-2 md:p-4 border-t border-border">
          <div className="flex justify-between items-center mb-3">
            <div className="text-xs text-muted-foreground flex items-center">
              ...
              <span>Tip: Ask me to show you an image or draw something!</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Input
